...
class HeaderSystem {
  ...
  displaySearchResults(results) {
    ...
    resultsCount.textContent = `${results.length} resultado${
      results.length !== 1 ? "s" : ""
    }`;
  ...
  setupNotifications() {
    const notificationBtn =
      this.notifications?.querySelector(".notification-btn");
  ...
  setupBreadcrumbs() {
    const breadcrumbItems =
      this.breadcrumb?.querySelectorAll(".breadcrumb-item a");
  ...
  setupKeyboardShortcuts() {
    document.addEventListener("keydown", (e) => {
      ...
      if ((e.ctrlKey || e.metaKey) && e.key === "k") {
        e.preventDefault();
        const searchInput =
          this.searchContainer?.querySelector(".search-input");
        searchInput?.focus();
      }
  ...
  setPageTitle(title) {
    ...
    const activeItem = this.breadcrumb?.querySelector(
