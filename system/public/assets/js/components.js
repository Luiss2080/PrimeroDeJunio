...
class ComponentSystem {
    ...
    animateStatsCard(card) {
        ...
        const hasSuffix = valueElement.textContent.includes('+') || valueElement.textContent.includes('%');
    ...
    // ===== SISTEMA DE BOTONES =====
    setupButtons() {
        const buttons = document.querySelectorAll('.btn-primary, .btn-secondary, .btn-outline, .btn-danger');
    ...
    async submitForm(form) {
        ...
        try {
            ...
            if (result.success) {
                this.showFormSuccess(form, result.message || 'Formulario enviado correctamente');
                form.reset();
            } else {
                this.showFormError(form, result.message || 'Error al enviar el formulario');
            }
        } catch (error) {
    ...
    initializeAnimations() {
        ...
        const elementsToAnimate = document.querySelectorAll('.form-group, .input-group, .alert');
    ...
    addCard(cardData, container = document.body) {
        ...
        card.innerHTML = `
            <div class="system-card-header">
                ...
                ${cardData.badge ? `<span class="system-card-badge">${cardData.badge}</span>` : ''}
            </div>
            <div class="system-card-content">
                ${cardData.content}
            </div>
            ${cardData.footer ? `<div class="system-card-footer">${cardData.footer}</div>` : ''}
        `;
