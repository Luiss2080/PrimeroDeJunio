...
class SidebarSystem {
    ...
    setupNavigation() {
        this.navItems.forEach(item => {
            ...
            link.addEventListener('click', (e) => {
                if (link.getAttribute('href') === '#' || item.classList.contains('has-submenu')) {
                    e.preventDefault();
    ...
    // ===== PERFIL DE USUARIO =====
    setupUserProfile() {
        const profileContainer = this.sidebar?.querySelector('.user-profile-sidebar');
    ...
    navigatePrevious() {
        ...
        const allLinks = Array.from(this.sidebar?.querySelectorAll('.nav-link') || []);
    ...
    addMenuItem(menuData, parentSelector = '.sidebar-nav .nav-menu') {
        ...
        menuItem.innerHTML = `
            <a href="${menuData.url || '#'}" class="nav-link">
                ...
                ${menuData.badge ? `<span class="nav-badge">${menuData.badge}</span>` : ''}
                ${menuData.submenu ? '<i class="nav-arrow fas fa-chevron-right"></i>' : ''}
            </a>
            ${menuData.submenu ? `
                <ul class="submenu">
