...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Navigation links
  document
    .querySelectorAll(".nav-link:not(.submenu-toggle)")
    .forEach((link) => {
  ...
  function toggleSubmenu(submenuId) {
    const navItem = document
      .querySelector(`[data-submenu="${submenuId}"]`)
      ?.closest(".nav-item");
  ...
  document.querySelectorAll(".nav-link, .submenu-link").forEach((link) => {
    link.addEventListener("click", function () {
      ...
      const section =
        this.closest(".nav-item")?.querySelector(".nav-text")?.textContent;
...
function updateNavBadge(navKey, count) {
  const navItem = document
    .querySelector(`[data-submenu="${navKey}"]`)
    ?.closest(".nav-item");
  if (!navItem) return;
